...
struct Restaurant {
    dust_count: f32, // airborne particles (0-200)
    avg_rating: f32, // average customer rating (0-5 stars)
}

#[test]
fn multiple_metrics_weighted_sum() -> Result<(), &'static str> {
    // "cleanliness": less dust → higher score.  Invert raw count.
    let m1 = metric32("cleanliness")
        .measure()
    ...
    let m2 = metric32("popularity")
        .measure()
        .by(|r: &Restaurant| r.avg_rating)
        .map01()
        .linear(5.0);

    let scorer = ScorerBuilder32::new().add(1.0, m1).add(3.0, m2).build()?;

    let r = Restaurant {
        dust_count: 20.0,
        avg_rating: 4.0,
    };
    let total = ScoreSet32::score(&scorer, &r);
...
struct DnaSequence {
    gc_bases: f32,    // number of G + C bases
    total_bases: f32, // sequence length in bases
}
...
fn breakdown_matches_score() -> Result<(), &'static str> {
    ...
    let length_score = metric32("length")
        .measure()
    ...
    let rows = ScoreSet32::breakdown(&scorer, &dna);
...
fn build_dna_scorer(
    cfg: &DnaScorerConfig,
) -> Result<Scorer32<DnaSequence, impl MetricTuple32<DnaSequence>>, &'static str> {
    let gc = {
        let t = cfg.gc_threshold;
        metric32("gc_filtered")
            .measure()
...
fn partial_application_dna_scorer() -> Result<(), &'static str> {
    ...
    let s1 = ScoreSet32::score(&scorer, &seq1);
    let s2 = ScoreSet32::score(&scorer, &seq2);
    ...
    let s_strict = ScoreSet32::score(&strict_scorer, &seq1);
    // gc = 0.8 < 0.9 → gc_filtered contributes 0
    assert!(s_strict < s1);

    // Breakdown on the original scorer
    let rows = ScoreSet32::breakdown(&scorer, &seq1);
    assert_eq!(rows.len(), 2);
