...
fn infinite_weight_rejected() {
    ...
    assert!(ScoreSet32::new().push(f32::INFINITY, m).is_err());
}
...
fn neg_infinite_weight_rejected() {
    ...
    assert!(ScoreSet32::new().push(f32::NEG_INFINITY, m).is_err());
}
...
fn sum_skips_failed_metric() -> Result<(), &'static str> {
    ...
    let total = ScoreSet32::new().push(1.0, good)?.push(1.0, bad)?.sum()?(&0.0);
...
fn single_element_normalization() -> Result<(), &'static str> {
    ...
    let total = ScoreSet32::new().push(5.0, m)?.sum()?(&0.7);
...
fn single_element_breakdown_weight_is_one() -> Result<(), &'static str> {
    ...
    let rows = ScoreSet32::new().push(3.0, m)?.breakdown()?(&0.4);
...
fn extreme_weights_normalize() -> Result<(), &'static str> {
    ...
    // f32::MIN_POSITIVE and f32::MAX are both finite and > 0
    let total = ScoreSet32::new()
        .push(f32::MIN_POSITIVE, m1)?
        .push(f32::MAX, m2)?
        .sum()?(&0.0);
    ...
    // Actually: min_pos / (min_pos + max) ≈ 0 for f32. So weight1 ≈ 0, weight2 ≈ 1.
    // contribution = 1.0*tiny_prob + 0.0*large_prob ≈ tiny_prob.
    assert!(total.is_finite());
    // With f32::MAX, the sum is dominated by MAX so tiny weight is ~0
    assert!(total < 1e-6);
    Ok(())
}
...
fn multiple_metrics_normalize() -> Result<(), &'static str> {
    let mut builder = ScoreSet32::new();
    for _ in 0..8 {
        let m = metric32("n")
            .measure()
            .by(|ctx: &f32| *ctx)
