...
fn sum_skips_failed_metric() -> Result<(), &'static str> {
    let good = metric32("good")
        .measure()
        .by(|_: &f32| 1.0)
        .map01()
        .identity();

    let bad = metric32("bad")
        .measure()
        .by(|_: &f32| 0.5)
        .map01()
        .by(|_| 1.5);
    ...
    let scorer = ScorerBuilder32::new()
        ...
        .add(1.0, bad)
        .build()?;
    let total = ScoreSet32::score(&scorer, &0.0);
...
fn breakdown_defaults_failed_metric_to_zero() -> Result<(), &'static str> {
    ...
    let scorer = ScorerBuilder32::new()
        ...
        .add(1.0, bad)
        .build()?;
    let rows = ScoreSet32::breakdown(&scorer, &0.0);
...
fn single_element_normalization() -> Result<(), &'static str> {
    ...
    let scorer = ScorerBuilder32::new().add(5.0, m).build()?;
    let total = ScoreSet32::score(&scorer, &0.7);
...
fn single_element_breakdown_weight_is_one() -> Result<(), &'static str> {
    ...
    let scorer = ScorerBuilder32::new().add(3.0, m).build()?;
    let rows = ScoreSet32::breakdown(&scorer, &0.4);
...
fn extreme_weights_normalize() -> Result<(), &'static str> {
    let m1 = metric32("tiny")
        .measure()
        .by(|_: &f32| 1.0)
        .map01()
        .identity();

    let m2 = metric32("huge")
        .measure()
        .by(|_: &f32| 0.0)
        .map01()
        .identity();
...
fn multiple_metrics_normalize() -> Result<(), &'static str> {
    ...
    let total = ScoreSet32::score(&scorer, &0.5);
    assert!(total.is_finite());
    assert!((total - 0.5).abs() < 1e-6);
    Ok(())
}
...
fn multiple_metrics_breakdown_consistent() -> Result<(), &'static str> {
    ...
    let scorer = ScorerBuilder32::new()
        ...
        .add(1.0, m.clone())
        .build()?;
    let rows = ScoreSet32::breakdown(&scorer, &0.3);
    ...
    let total: f32 = rows.iter().map(|r| r.contribution).sum();
...
fn heterogeneous_capturing_closures_deterministic() -> Result<(), &'static str> {
    ...
    // A) captures Vec<f32> (owned, non-Copy) — proves Fn bound is satisfied
    // B) captures f32 threshold (Copy) — different F type from A
    // C) non-capturing — a third distinct F type

    let owned_data = alloc::vec![1.0_f32, 2.0, 3.0];
    ...
    let with_threshold = {
        ...
        metric32("threshold")
            .measure()
            .by(move |ctx: &f32| if *ctx > t { 1.0 } else { 0.0 })
            .map01()
            .identity()
    };

    let non_capturing = metric32("plain")
        .measure()
        .by(|ctx: &f32| 1.0 - ctx)
        .map01()
        .identity();
    ...
    let r1 = ScoreSet32::score(&scorer, &0.8);
    let r2 = ScoreSet32::score(&scorer, &0.8);
    assert!((r1 - r2).abs() < 1e-9);

    // Different ctx → different result
    let lo = ScoreSet32::score(&scorer, &0.0);
    let hi = ScoreSet32::score(&scorer, &1.0);
    assert!(hi > lo);

    // Breakdown matches score
    let rows = ScoreSet32::breakdown(&scorer, &0.8);
...
fn capturing_closure_repeated_eval_proves_fn_bound() -> Result<(), &'static str> {
    // Captures an owned Vec<f32> — non-Copy.
    // Repeated eval via shared ref proves `F: Fn` (not just FnOnce).
    let captured = alloc::vec![10.0_f32, 20.0];
    let m = metric32("captured")
        .measure()
        .by(move |ctx: &f32| {
    ...
    let scorer = ScorerBuilder32::new().add(3.0, m).build()?;
    let r1 = ScoreSet32::score(&scorer, &0.5);
    let r2 = ScoreSet32::score(&scorer, &0.5);
