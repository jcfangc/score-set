...
fn normalize_equal_weights() -> Result<(), &'static str> {
    let m1 = metric("a")
        .measure().by(|v: f64| v)
        .map01().by(|raw: &f64, _: f64| (*raw).witness().by(|v| Value01::prove(*v)).unwrap())
        .build();
    ...
    let ms = score_set! { 1.0 => m1, 1.0 => m2 }?.normalize()?;
    let score = ms
        .score()
...
fn normalize_single_member() -> Result<(), &'static str> {
    ...
    let ms = score_set! { 5.0 => m }?.normalize()?;
...
fn normalize_with_f32() -> Result<(), &'static str> {
    let m = metric("f32-m")
        .measure().by(|v: f32| v)
        .map01().by(|raw: &f32, _: f32| (*raw).witness().by(|v| Value01::prove(*v)).unwrap())
        .build();

    let ms = score_set! { 2.0_f32 => m }?.normalize()?;
