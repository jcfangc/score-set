...
// ---- 测度函数（fn 指针，可命名，能放入 finite_metric! 枚举变体） ----
...
// 类型别名（必须用 fn 指针才能在 finite_metric! 枚举变体中使用）
type GcMetric =
...
fn layer1_full_flow() -> Result<(), &'static str> {
    ...
    // Step 2: 构建 MetricSet — weight => metric 一对一
    let set = fixed_score_set! {
        2.0 => gc_metric(),
        3.0 => len_metric(),
    }?;

    // Step 3: 评分 — .score() 零参数，.by() 拿元组引用，惰性求值
    ...
    // GC=6/12=0.5, len=12/100=0.12
    // weights: gc=0.4, len=0.6 → 0.4*0.5 + 0.6*0.12 = 0.272
    ...
    assert!((worst - 0.6 * 0.12).abs() < 1e-10); // len 贡献更小: 0.6*0.12=0.072

    Ok(())
}

// ===========================================================================
// Layer 2 — 有限枚举：finite_metric! 声明 + finite_score_set! 宏
// ===========================================================================

// Step 1: 声明指标枚举（放在模块顶部，一次声明可复用）
finite_metric! {
    DnaMetric(f64, String) =>
        Gc(GcMetric),
        Len(LenMetric),
    // Custom(Box<dyn DynMetric<f64, String>>) 自动生成
}

#[test]
fn layer2_full_flow() -> Result<(), &'static str> {
    // Step 2: 构建 MetricSet — weight => variant
    let set = finite_score_set! {
        2.0 => DnaMetric::Gc(gc_metric()),
        3.0 => DnaMetric::Len(len_metric()),
    }?;

    // Step 3: 评分 — 和 Layer 1 完全一样！.score() 零参数，.by() 拿切片
    let dna = "ACGTACGTACGT".to_string();
    let total = set.score().by(|members: &[FiniteMember<f64, DnaMetric>]| {
        members
            .iter()
...
// ===========================================================================
// Layer 3 — 完全动态：dynamic_score_set! 宏
// ===========================================================================

#[test]
fn layer3_full_flow() -> Result<(), &'static str> {
    // Step 1+2: 构建 MetricSet — weight => metric.boxed()
    let set = dynamic_score_set! {
        2.0 => gc_metric().boxed(),
        3.0 => len_metric().boxed(),
    }?;

    // Step 3: 评分 — 和 Layer 1/2 完全一样！
    let dna = "ACGTACGTACGT".to_string();
    let total = set.score().by(|members: &[DynamicMember<f64, String>]| {
        members
            .iter()
            .fold(0.0, |acc, m| acc + m.contribute(m.metric().eval(&dna)))
    });
